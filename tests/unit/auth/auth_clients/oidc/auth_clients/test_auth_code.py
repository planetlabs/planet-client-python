...
def mocked_authapi_get_authcode(
        obj_self, client_id, redirect_uri, requested_scopes,
        pkce_code_challenge):
    return TEST_AUTH_CODE


def mocked_tokenapi_token_from_code(
        obj_self, redirect_uri, client_id, code, code_verifier):
    return MOCK_TOKEN
...
class PkceAuthCodeFlowTest(unittest.TestCase):
    ...
    def setUp(self):
        self.under_test = AuthCodePKCEAuthClient(
            AuthCodePKCEClientConfig(auth_server=TEST_AUTH_SERVER,
                                     token_endpoint=TEST_AUTH_SERVER + '/token',
                                     authorization_endpoint=TEST_AUTH_SERVER + '/auth',
                                     client_id=TEST_CLIENT_ID,
                                     redirect_uri=TEST_RECIRECT_URI_LOCAL))

    @mock.patch('planet.auth.oidc.api_clients.authorization_api_client.AuthorizationAPIClient.authcode_from_pkce_flow_with_browser_with_callback_listener', mocked_authapi_get_authcode)  # noqa
    @mock.patch('planet.auth.oidc.api_clients.token_api_client.TokenAPIClient.get_token_from_code', mocked_tokenapi_token_from_code)  # noqa
    ...
    @mock.patch('planet.auth.oidc.api_clients.authorization_api_client.AuthorizationAPIClient.authcode_from_pkce_flow_without_browser_without_callback_listener', mocked_authapi_get_authcode)  # noqa
    @mock.patch('planet.auth.oidc.api_clients.token_api_client.TokenAPIClient.get_token_from_code', mocked_tokenapi_token_from_code)  # noqa
    def test_login_no_browser(self):
        ...
        test_result = self.under_test.login(
            allow_open_browser=False, requested_scopes=['override'])
        self.assertIsInstance(test_result, FileBackedOidcToken)
        self.assertEqual(MOCK_TOKEN, test_result.data())
    ...
    def test_default_request_authenticator_type(self):
        ...
        self.assertIsInstance(
            test_result, RefreshingOidcTokenRequestAuthenticator)
