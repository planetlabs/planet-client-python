...
class RefreshingOidcTokenAuth(BearerTokenAuth):
    """
    ...
    This class assumes access tokens are JWTs, which OIDC and OAuth do not require.
    ...
    def _load(self):
        # Absolutely not appropriate to not verify the signature in a token validation
        # context (e.g. server side auth of a client). Here we know that's not what
        # we are doing. This is a client helper class for clients who will be
        # presenting tokens to such a server.  We are inspecting ourselves, not verifying
        # for trust purposes.  We are not expected to be the audience.
        self._token.load()
    ...
    def __call__(self, r):
        if int(time.time()) > self._refresh_at:
            try:
                # Reload the file before refreshing. Another process might have done it for us,
                # and save us the network call.  Also, if refresh tokens are configured to be
                # one time use, we want a fresh refresh token. Stale refresh tokens are invalid
                # in this case.
                self._load()
                if int(time.time()) > self._refresh_at:
                    self._refresh()
            except OIDCAPIClientException as e:
                # we continue with the old auth token to try and be resilient. Refresh failures could be transient.
                logger.warning(
...
class RefreshOrReloginOidcTokenAuth(RefreshingOidcTokenAuth):
    """
    Decorate a http request with a bearer auth token. Automatically initiate a refresh request using the
    refresh token if we know the access token to be close to expiration. If we do not have a refresh token,
    then fall back to re-initiating a login.  Sometimes (like with client credential flow), refresh
    tokens may not be available and we might want to login rather than refresh.  It is not an automatic choice
    to fall back to login when refresh is not available, since for some auth client configurations login is
