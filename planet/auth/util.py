...
class FileBackedJsonObject:
    ...
    lazy loading and validation before saving the data.  Derived classes
    should provide type specific validation and convenience data accessors.
    """
    ...
    def assert_valid(self):
        ...
        if not self._data:
            raise FileBackedJsonObjectException(
                "Data has not been successfully loaded from " +
                str(self._file_path))
    ...
    def load(self):
        ...
        # Open to debate if this is best, but if there is a failure, the
        # _data reflects this by being empty. This forces repeated lazy loads
        # to retry.  Alternatively, we could throw and leave data as-is, but
        # then lazy load would not retry, only explicit loads would.
        self._data = None
    ...
    def save(self):
        ...
        with open(self._file_path, 'w') as token_file_w:
            ...
            # Add a few seconds of wiggle room to sync the file and not
            # trigger a reload.
    ...
    # We may not want to reload in the middle of a transaction that needs to
    # get multiple fields. It's up to the user to know when to user a lazy
    # load vs lazy reload.
